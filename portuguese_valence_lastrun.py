...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    slider = visual.Slider(win=win, name='slider',
        startValue=None, size=(1200, 50), pos=(0, -100), units=win.units,
        labels=("1- Muito Negativo", "2", "3", "4", "5", "6", "7"), ticks=(1, 2, 3, 4, 5, 6, 7), granularity=1.0,
        style='rating', styleTweaks=[], opacity=None,
        labelColor='Black', markerColor='Red', lineColor='Black', colorSpace='rgb',
        font='Open Sans', labelHeight=25.0,
        flip=False, ori=0.0, depth=-1, readOnly=False)
    mouse_valence = event.Mouse(win=win)
    x, y = [None, None]
    mouse_valence.mouseClock = core.Clock()
    ...
    for thisTrial_2 in trials_2:
        ...
        treino = data.Routine(
            name='treino',
            components=[stim_treino, slider_treino, mouse],
        )
    ...
    for thisTrial in trials:
        ...
        valence = data.Routine(
            name='valence',
            components=[stim_text, slider, mouse_valence],
        )
        valence.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        slider.reset()
        # setup some python lists for storing info about the mouse_valence
        mouse_valence.x = []
        mouse_valence.y = []
        mouse_valence.leftButton = []
        mouse_valence.midButton = []
        mouse_valence.rightButton = []
        mouse_valence.time = []
        gotValidClick = False  # until a click is received
        ...
        while continueRoutine:
            ...
            if slider.status == STARTED:
                # update params
                pass
            # *mouse_valence* updates
            
            # if mouse_valence is starting this frame...
            if mouse_valence.status == NOT_STARTED and t >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                mouse_valence.frameNStart = frameN  # exact frame index
                mouse_valence.tStart = t  # local t and not account for scr refresh
                mouse_valence.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(mouse_valence, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.addData('mouse_valence.started', t)
                # update status
                mouse_valence.status = STARTED
                mouse_valence.mouseClock.reset()
                prevButtonState = mouse_valence.getPressed()  # if button is down already this ISN'T a new click
            
            # if mouse_valence is stopping this frame...
            if mouse_valence.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > mouse_valence.tStartRefresh + 15-frameTolerance:
                    # keep track of stop time/frame for later
                    mouse_valence.tStop = t  # not accounting for scr refresh
                    mouse_valence.tStopRefresh = tThisFlipGlobal  # on global time
                    mouse_valence.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.addData('mouse_valence.stopped', t)
                    # update status
                    mouse_valence.status = FINISHED
            if mouse_valence.status == STARTED:  # only update if started and not finished!
                buttons = mouse_valence.getPressed()
                if buttons != prevButtonState:  # button state changed?
                    prevButtonState = buttons
                    if sum(buttons) > 0:  # state changed to a new click
                        pass
                        x, y = mouse_valence.getPos()
                        mouse_valence.x.append(float(x))
                        mouse_valence.y.append(float(y))
                        buttons = mouse_valence.getPressed()
                        mouse_valence.leftButton.append(buttons[0])
                        mouse_valence.midButton.append(buttons[1])
                        mouse_valence.rightButton.append(buttons[2])
                        mouse_valence.time.append(mouse_valence.mouseClock.getTime())
        ...
        trials.addData('mouse_valence.x', mouse_valence.x)
        trials.addData('mouse_valence.y', mouse_valence.y)
        trials.addData('mouse_valence.leftButton', mouse_valence.leftButton)
        trials.addData('mouse_valence.midButton', mouse_valence.midButton)
        trials.addData('mouse_valence.rightButton', mouse_valence.rightButton)
        trials.addData('mouse_valence.time', mouse_valence.time)
